...
How to version control your dotfiles using git.
...
```
    ...
    dotfiles remote add origin git@github.com:dylan-k/dotfiles.git
```

where my ~/.dotfiles directory is a git bare repository. 

Thanks to `showUntrackedFiles no` it will ignore everything unless you specifically add it later. This prevents lots of clutter, as the home directory filles with everything.

The alias will be used instead of typical git commands for this special repository. For example, instead of `git status` do `dotfiles status`, and you can do this from within any directory.
...
To replicate Dotfiles on Another Machine...

```
git clone --separate-git-dir=$HOME/.my-dotfiles https://github.com/dylan-k/dotfiles.git my-dotfiles-tmp
rsync --recursive --verbose --exclude '.git' my-dotfiles-tmp/ $HOME/
rm --recursive my-dotfiles-tmp
dotfiles config status.showUntrackedFiles no
dotfiles remote set-url origin git@github.com:dylan-k/dotfiles.git
```
